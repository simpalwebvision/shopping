...
class AMCGSTFormPage extends StatefulWidget {
  final ByteData byteList;

  final List<AddPartModel> finallist;
  const AMCGSTFormPage(
      {super.key, required this.byteList, required this.finallist});

  @override
  State<AMCGSTFormPage> createState() => _AMCGSTFormPageState();
}

class _AMCGSTFormPageState extends State<AMCGSTFormPage> {
                      ...
                      child: Column(
                        children: [
                          ...
                          const SmallTermsAndConditionWidget(
                              ...
                              text:
                                  'Filtration parts will be replace once in a year.'),
                          SizedBox(height: size.height * 0.005),
                          const SmallTermsAndConditionWidget(
                              initialText: '2.',
                              iconColor: Colors.black,
                              text:
                                  'Three Service in a year = PP candle change.'),
                          SizedBox(height: size.height * 0.005),
                          const SmallTermsAndConditionWidget(
                              initialText: '3.',
                              iconColor: Colors.black,
                              text:
                                  'The Company will Resolve All the issue during warranty period.'),
                          SizedBox(height: size.height * 0.005),
                          const SmallTermsAndConditionWidget(
                              initialText: '4.',
                              iconColor: Colors.black,
                              text:
                                  'Cabinet body is not included in any in CMC Contract.'),
                          SizedBox(height: size.height * 0.01),
                          const SmallTermsAndConditionWidget(
                              initialText: '5.',
                              iconColor: Colors.black,
                          ...
                          const BigTermsAndConditionWidget(
                              ...
                              iconColor: Colors.black,
                              text:
                                  'Cheque Payment in favour of "AQUA NERIO".'),
                          SizedBox(height: size.height * 0.01),
                          Padding(
                            padding: EdgeInsets.symmetric(
                                horizontal: size.width * 0.02),
...
class SmallTermsAndConditionWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    var size = MediaQuery.of(context).size;
    return Padding(
      padding: EdgeInsets.symmetric(horizontal: size.width * 0.02, vertical: 0),
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
...
class BigTermsAndConditionWidget extends StatelessWidget {
  final String text, initialText;
  final Color? iconColor;
  const BigTermsAndConditionWidget(
      {Key? key,
...
class AmoountHelperWidget extends StatelessWidget {
  final Size sSize;
  final String headText, value;

  const AmoountHelperWidget(
      {super.key,
      required this.sSize,
  ...
  Widget build(BuildContext context) {
    return Padding(
      padding: EdgeInsets.symmetric(horizontal: sSize.width * 0.01),
      child: Row(
        children: [
          Expanded(
            flex: 2,
...
class CompanyNameHelper extends StatelessWidget {
  final Size sSize;
  final String headText, value;

  const CompanyNameHelper(
      {super.key,
      required this.sSize,
      required this.headText,
      required this.value});
