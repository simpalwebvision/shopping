...
class BounceProvider with ChangeNotifier {
  ...
  double get endAmount => _endAmount;
  double get directDiscAmt => _directDiscAmt;
  ...
  void addPartsFunction({
    ...
    _endAmount = _directDiscAmt + _total;
    if (discount != "") {
  ...
  void directDiscAmtMethod(value) {
    _directDiscAmt = CommonFunctions.discountOnChangedFunction(value, 2500);
  ...
  void discountOnChangedFunction(value) {
    _totalAmount = CommonFunctions.discountOnChangedFunction(value, _endAmount);
    if (_gstTotalAmt != 0.0) {
      _gstTotalAmt = 0.0;
    }
    notifyListeners();
  ...
  void onAddButtonClick(tmpValue) {
    ...
    if (temporary != "") {
      if (_totalAmount == 0.0) {
        var amt = (_endAmount * int.parse(temporary)) / 100;
        _gstTotalAmt = _endAmount + amt;
      } else {
  ...
  void onSubtractButtonClick(tmpValue) {
    ...
    if (temporary != "") {
      if (_totalAmount == 0.0) {
        var amt =
            (_endAmount - (_endAmount * (100 / (100 + int.parse(temporary)))));
        _gstTotalAmt = _endAmount - amt;
      } else {
  ...
  Future<void> navigateToSignaturePad(
      BuildContext context, BounceProvider provider) async {
